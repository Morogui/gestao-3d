...
//
// 2026-08-26: a sessão da Shopee (access_token/refresh_token/shop_id)
// passou a morar no banco (tabela shopee_auth), não mais em cookie —
// mesmo fix já aplicado à ML em 2026-08-14 (lib/ml-auth.ts). Causa raiz
// do "Shopee caindo toda hora": getValidShopeeAccessToken() antes lia
// cookie httpOnly, que o cron de 1 em 1 minuto (vercel.json) nunca
// enxergava (chamada servidor-a-servidor, sem navegador nenhum por
// trás) — então a sincronização automática da Shopee nunca funcionava
// de verdade, só quando alguém abria a aba Estoque/Vendas com o cookie
// do navegador ainda vivo (o access_token da Shopee dura só 4h). Ver
// lib/shopee-auth.ts pro detalhe completo e a nova implementação
// (DB-first, com auto-refresh persistido de volta no banco).
import { getValidShopeeAccessToken } from "./shopee-auth";
...
// order_status (sem item_list, buyer, shipping) — usada só pra somar
// faturamento por dia (recorde de melhor dia da loja em até 90 dias).
// Mesmo espírito da getDailyTotalsRange da ML em lib/ml-orders.ts: evita
// o custo de buscar item_list + foto de cada pedido quando só precisamos
// do total do dia.
//
// 2026-09-08: passou a pedir order_status também — precisa dele pra
// filtrar fora UNPAID/CANCELLED/IN_CANCEL antes de somar (ver comentário
// em getDailyTotalsRange abaixo).
async function fetchOrderTotalsOnly(
    orderSns: string[],
    accessToken: string,
    shopId: number
  ): Promise<{ create_time?: number; total_amount?: number; order_status?: string }[]> {
    const details: { create_time?: number; total_amount?: number; order_status?: string }[] = [];
    for (let i = 0; i < orderSns.length; i += 50) {
          const batch = orderSns.slice(i, i + 50);
          const url = new URL(
                  signAuthenticatedRequest("/api/v2/order/get_order_detail", accessToken, shopId)
          ...
          url.searchParams.set("order_sn_list", batch.join(","));
          url.searchParams.set("response_optional_fields", "total_amount,create_time,order_status");
          const resp = await fetch(url.toString(), { cache: "no-store" });
          if (!resp.ok) continue;
          const data = await resp.json();
          const list: { create_time?: number; total_amount?: number; order_status?: string }[] =
...
// (fetchOrderSnsInRange) e só busca total/data/status de cada pedido —
// sem item_list nem foto — pra não pesar a página.
//
// 2026-09-08: filtra por NAO_VENDIDO_SHOPEE antes de somar — antes
// somava TODO pedido devolvido pela busca (incluindo UNPAID/CANCELLED/
// IN_CANCEL), inflando o "Recorde da loja"/"Recorde do mês" muito além
// do que a própria Shopee mostra no painel dela (que já filtra "Produto
// Pago" por padrão).
export async function getDailyTotalsRange(
  ...
  try {
      ...
      const totals = await fetchOrderTotalsOnly(orderSns, accessToken, shopId);
        const vendidos = totals.filter(
                (o) => !NAO_VENDIDO_SHOPEE.has(o.order_status ?? "")
              );
