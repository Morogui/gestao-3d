...
//
// 2026-09-08: exportados (antes eram `const` locais) — a aba Vendas
// estava somando TODO pedido cacheado (qualquer status: pendente de
// pagamento, cancelado, etc.) nos cards de resumo/ranking/recorde, o que
// inflava os números muito além do que a própria ML/Shopee mostram nos
// paineis delas (só contam venda de verdade). getDailyTotalsRange (ML,
// abaixo) e a versão Shopee em lib/shopee-orders.ts agora reusam esses
// mesmos conjuntos pra filtrar antes de somar por dia.
export const VENDIDO_ML = new Set(["paid", "partially_paid"]);
export const NAO_VENDIDO_SHOPEE = new Set(["UNPAID", "CANCELLED", "IN_CANCEL"]);
...
// /orders/search (id, data, total, status), sem o multiget de /items
// (foto/SKU) nem a chamada de /shipments por pedido — o que faz a
// diferença entre levar 1 chamada de API vs. 1 + N (N = nº de pedidos)
// chamadas. Isso é o que permite varrer 90 dias de histórico sem
// estourar o timeout da função serverless. As páginas são buscadas em
// paralelo (depois de descobrir o total na primeira página) pra reduzir
// ainda mais o tempo de resposta.
//
// 2026-09-08: filtra por VENDIDO_ML antes de somar — antes somava TODO
// pedido retornado pela busca (incluindo payment_required/cancelled/
// invalidated/pending_cancel), o que fazia o "Recorde da loja" e o
// "Recorde do mês" contarem pedidos que nunca viraram venda de verdade.
export async function getDailyTotalsRange(
  ...
  // Só conta como venda de verdade (mesma regra de pedidoFoiVendido) —
