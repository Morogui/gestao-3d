...
async function ensureTable() {
  ...
  // 14/09/2026 -- custo periodico de armazenagem do Mercado Envios Full
  // (nao aparece por pedido individual, e cobrado no faturamento mensal
  // do ML). Default 0 ate o valor real ser confirmado na fatura.
...
function toConfig(row: ConfigRow): ConfigPrecificacao {
  return {
    ...
    afiliadoPctML: Number(row.afiliado_pct_ml),
    ...
    armazenagemFullML: Number(row.armazenagem_full_ml),
  };
}
  ...
  if (existing.length > 0) {
    rows = (await sql`
      ...
      SET imposto_pct = ${impostoPct},
          ...
          armazenagem_full_ml = ${armazenagemFullML},
          atualizado_em = now()
      WHERE id = ${existing[0].id}
      RETURNING imposto_pct, ads_pct_ml, ads_pct_shopee, afiliado_pct_ml, afiliado_pct_shopee, embalagem_custo, margem_desejada_pct, reembolso_flex_ml, custo_flex_ml, armazenagem_full_ml
    `) as ConfigRow[];
  } else {
    rows = (await sql`
      INSERT INTO precificacao_config (imposto_pct, ads_pct_ml, ads_pct_shopee, afiliado_pct_ml, afiliado_pct_shopee, embalagem_custo, margem_desejada_pct, reembolso_flex_ml, custo_flex_ml, armazenagem_full_ml)
      VALUES (${impostoPct}, ${adsPctML}, ${adsPctShopee}, ${afiliadoPctML}, ${afiliadoPctShopee}, ${embalagemCusto}, ${margemDesejadaPct}, ${reembolsoFlexML}, ${custoFlexML}, ${armazenagemFullML})
      RETURNING imposto_pct, ads_pct_ml, ads_pct_shopee, afiliado_pct_ml, afiliado_pct_shopee, embalagem_custo, margem_desejada_pct, reembolso_flex_ml, custo_flex_ml, armazenagem_full_ml
    `) as ConfigRow[];
  }
  return NextResponse.json(toConfig(rows[0]));
