...
let rows: ConfigRow[];
  if (existing.length > 0) {
    ...
    RETURNING imposto_pct, ads_pct_ml, ads_pct_shopee, afiliado_pct_shopee, embalagem_custo, margem_desejada_pct
    `) as ConfigRow[];
  } else {
    rows = (await sql`
    INSERT INTO precificacao_config (imposto_pct, ads_pct_ml, ads_pct_shopee, afiliado_pct_shopee, embalagem_custo, margem_desejada_pct)
    VALUES (${impostoPct}, ${adsPctML}, ${adsPctShopee}, ${afiliadoPctShopee}, ${embalagemCusto}, ${margemDesejadaPct})
    RETURNING imposto_pct, ads_pct_ml, ads_pct_shopee, afiliado_pct_shopee, embalagem_custo, margem_desejada_pct
    `) as ConfigRow[];
  }
  return NextResponse.json(toConfig(rows[0]));
