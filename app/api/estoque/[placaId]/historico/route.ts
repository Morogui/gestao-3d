...
export async function GET(
    ...
    const [vendas, manuais, producoesProprio, producoesExtra] = await Promise.all([
          sql`
            ...
            LIMIT 100
          ` as Promise<{ data: string; plataforma: string; pedido_id: string; pecas: number }[]>,
          ...
          sql`
            ...
            ORDER BY criado_em DESC
            LIMIT 100
          ` as Promise<{ data: string; delta: number; resultante: number }[]>,
          ...
          sql`
            ...
            LIMIT 100
          ` as Promise<
            { id: number; data: string; quantidade_placas: string; saida_extra_pecas: string }[]
          >,
        ]);
